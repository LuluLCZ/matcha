			...
			if (row.lenght != 0) {
				if (interest == 'female' && gender == 'male') {
					connect.query('SELECT login, gender, fname, lname, age, interest, sumup, profpic FROM users WHERE login != ? AND gender = ? AND interest = ?', [login, 'female', 'male'], function(err, row) {
						if (err) console.log(err)
						var profil = row
						res.render('home', { title: 'Home', profil: profil, filter: "New users" })
					})
				}
		...
		connect.query('SELECT * FROM users WHERE login != ?', [login], function(err, row, result) {
			...
			if (row.lenght != 0) {
				if (interest == 'female' && gender == 'male') {
					...
					connect.query('SELECT login, gender, fname, lname, age, interest, sumup, profpic FROM users WHERE login != ? AND gender = ? AND interest = ? ORDER BY popu', [login, 'male', 'female'], function(err, row) {
						if (err) console.log(err)
						var profil = row
						res.render('home', { title: 'Home', profil: profil, filter: "popu" })
					})
					...
					connect.query('SELECT login, gender, fname, lname, age, interest, sumup, profpic FROM users WHERE login != ? AND interest = ? ORDER BY popu', [login, 'both'], function(err, row) {
						if (err) console.log(err)
						var profil = row
						res.render('home', { title: 'Home', profil: profil, filter: "popu" })
					})
					...
					connect.query('SELECT login, gender, fname, lname, age, interest, sumup, profpic FROM users WHERE login != ? AND gender = ? AND interest = ? ORDER BY popu', [login, 'male', 'male'], function(err, row) {
						if (err) console.log(err)
						var profil = row
						res.render('home', { title: 'Home', profil: profil, filter: "popu" })
					})
					...
					connect.query('SELECT login, gender, fname, lname, age, interest, sumup, profpic FROM users WHERE login != ? AND gender = ? AND interest = ? ORDER BY popu', [login, 'female', 'female'], function(err, row) {
						if (err) console.log(err)
						var profil = row
						res.render('home', { title: 'Home', profil: profil, filter: "popu" })
					})
